...
class ChainValidateView(APIView):
    ...
    # AFTER (ChainValidateView)
def get(self, request):
    cached = cache.get(CHAIN_VALID_CACHE_KEY)
    if cached is None:
        is_valid, message = validate_chain()
        cache.set(CHAIN_VALID_CACHE_KEY, (is_valid, message), CHAIN_VALID_CACHE_TTL)
    else:
        is_valid, message = cached
...
class ChainStatsView(APIView):
    ...
    def get(self, request):
        ...
        # AFTER (ChainStatsView)
        cached = cache.get(CHAIN_VALID_CACHE_KEY)
        if cached is None:
            is_valid, _ = validate_chain()
            cache.set(CHAIN_VALID_CACHE_KEY, (is_valid, _), CHAIN_VALID_CACHE_TTL)
        else:
            is_valid, _ = cached
