...
document.querySelectorAll('a[href^="#"]').forEach(anchor => {
    anchor.addEventListener('click', function(e) {
        ...
        // Only intercept if the target section actually exists on this page
        if (target) {
            e.preventDefault();
        ...
        // If target doesn't exist, let the browser handle the href normally
    });
});
...
// Handle Back-Forward Cache (bfcache) to prevent blank screen on back button
...
document.addEventListener('click', function (e) {
    ...
    setTimeout(() => { 
        ...
        // Restore class immediately so bfcache stores the 'loaded' state
        setTimeout(() => {
            document.body.classList.add('loaded');
        }, 100);
    }, 340);
...
// Disable bfcache in Safari by attaching an empty unload listener
