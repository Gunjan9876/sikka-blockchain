...
class MiningRig(models.Model):
    ...
    Hash rate determines how quickly the PoW puzzle is solved.
    """
    ...
    tier = models.CharField(
        ...
        default=Tier.BASIC,
        help_text="Current hardware tier. Higher tier = more hash rate = faster PoW solve.",
    )
...
class MiningPool(models.Model):
    ...
    When a pool member solves a PoW puzzle and claims a block, the reward
    is split proportionally among all active pool members by hash rate.
    """
...
class MiningSession(models.Model):
    ...
    # ── PoW Race fields (Phase 2) ──────────────────────────────────────────────
    pow_nonce = models.BigIntegerField(
        default=0,
        help_text="Winning nonce found during PoW race.",
    )
    pow_hash = models.CharField(
        max_length=64,
        blank=True,
        default="",
        help_text="Block hash produced by this miner's PoW solution.",
    )
    pow_attempts = models.BigIntegerField(
        default=0,
        help_text="Total hashes tried before finding solution.",
    )
